    ...
    <div class="py-6 px-6">
        <div class="max-w-2xl mx-auto">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('admin.settings.object-subtypes.update', $objectSubtype) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <select name="objectType_id" 
                                ...
                                @foreach($objectTypes as $type)
                                    <option value="{{ $type->id }}" 
                                            {{ old('objectType_id', $objectSubtype->objectType_id) == $type->id ? 'selected' : '' }}>
                                        {{ $type->naam }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" 
                                   name="naam" 
                                   id="naam" 
                                   value="{{ old('naam', $objectSubtype->naam) }}"
                                   class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                   required>
                            @error('naam')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
